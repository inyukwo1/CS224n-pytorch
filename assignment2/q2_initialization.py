...
def xavier_weight_init():
    ...
    def _xavier_initializer(shape, **kwargs):
        ...
        """
        epsilon = np.sqrt(6 / np.sum(np.array(shape, dtype="float32"))) # Careful!!!!!!!!!!!
        out = torch.from_numpy(np.random.uniform(-epsilon, epsilon, shape))
        return out
