...
def test_softmax_basic():
    ...
    test1 = softmax(torch.from_numpy(np.array([[1001, 1002], [3, 4]], dtype="float32"))).numpy()
    test_all_close("Softmax test 1", test1, np.array([[0.26894142, 0.73105858],
                                                      [0.26894142, 0.73105858]]))

    test2 = softmax(torch.from_numpy(np.array([[-1001, -1002]], dtype="float32"))).numpy()
...
def test_cross_entropy_loss_basic():
    ...
    test1 = cross_entropy_loss(torch.from_numpy(y), torch.from_numpy(yhat)).numpy()
    expected = -3 * np.log(.5)
